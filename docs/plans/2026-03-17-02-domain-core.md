...
- [ ] **Step 3: Create constants.ts**
  ...
  export const CHILD_CATEGORIES: DomesticCategory[] = [
...
- [ ] **Step 1: Write failing tests**
  ...
  const allFifty: DomesticSliders = {
    groceries: 50, cooking: 50, cleaning: 50, admin: 50,
    childrenAppointments: 50, schoolSupport: 50, maintenance: 50, planning: 50,
  };

  const p1Heavy: DomesticSliders = {
    groceries: 80, cooking: 80, cleaning: 80, admin: 80,
    childrenAppointments: 80, schoolSupport: 80, maintenance: 20, planning: 80,
  };
  ...
  describe("mergeDomesticSliders", () => {
    ...
    it("returns midpoint when both p1 and p2 sliders are provided (couple mode)", () => {
      const p2Sliders: DomesticSliders = {
        groceries: 20, cooking: 20, cleaning: 20, admin: 20,
        childrenAppointments: 20, schoolSupport: 20, maintenance: 20, planning: 20,
      };
...
- [ ] **Step 3: Implement domestic.ts**
  ...
  export function mergeDomesticSliders(
    ...
    return Object.fromEntries(
      (Object.keys(p1) as DomesticCategory[]).map((key) => [
        key,
        (p1[key] + p2[key]) / 2,
      ])
    ) as DomesticSliders;
...
- [ ] **Step 1: Write failing tests**
  ...
  const base: SimulationInput = {
    p1: { name: "Thomas", income: 3200, personalCharges: 0, workQuota: 1, fullTimeIncome: 3200, partTimeReason: null },
    p2: { name: "Léa", income: 2100, personalCharges: 0, workQuota: 0.8, fullTimeIncome: 2625, partTimeReason: "couple-choice" },
    commonCharges: 3000,
    hasChildren: true,
    domesticSliders: { p1: { groceries: 35, cooking: 40, cleaning: 25, admin: 30, childrenAppointments: 20, schoolSupport: 35, maintenance: 75, planning: 30 } },
    hourlyRate: 9.57,
  };
...
- [ ] **Step 1: Write failing tests**
  ...
  const base = {
    p1: { name: "Thomas", income: 3200, personalCharges: 0, workQuota: 1, fullTimeIncome: 3200, partTimeReason: null },
    p2: { name: "Léa", income: 2100, personalCharges: 0, workQuota: 0.8, fullTimeIncome: 2625, partTimeReason: "couple-choice" as const },
    commonCharges: 3000,
    hasChildren: true,
    domesticSliders: { p1: { groceries: 50, cooking: 50, cleaning: 50, admin: 50, childrenAppointments: 50, schoolSupport: 50, maintenance: 50, planning: 50 } },
    hourlyRate: 9.57,
  };
...
- [ ] **Step 1: Write failing tests**
  ...
  const base = {
    p1: { name: "Thomas", income: 3200, personalCharges: 80, workQuota: 1, fullTimeIncome: 3200, partTimeReason: null },
    p2: { name: "Léa", income: 2100, personalCharges: 190, workQuota: 0.8, fullTimeIncome: 2625, partTimeReason: "couple-choice" as const },
    commonCharges: 3000,
    hasChildren: true,
    domesticSliders: { p1: { groceries: 50, cooking: 50, cleaning: 50, admin: 50, childrenAppointments: 50, schoolSupport: 50, maintenance: 50, planning: 50 } },
    hourlyRate: 9.57,
...
- [ ] **Step 3: Implement m4-adjusted-time.ts**
  ...
  export function computeM4(input: SimulationInput): M4Result {
    ...
    const isSameAsM2 =
      input.p1.workQuota === 1 && input.p2.workQuota === 1;
...
- [ ] **Step 1: Write failing tests**
  ...
  const allFifty = { groceries: 50, cooking: 50, cleaning: 50, admin: 50, childrenAppointments: 50, schoolSupport: 50, maintenance: 50, planning: 50 };
  const p1Heavy = { groceries: 80, cooking: 80, cleaning: 80, admin: 80, childrenAppointments: 80, schoolSupport: 80, maintenance: 20, planning: 80 };
  ...
  describe("Model 5 — Total contribution", () => {
    ...
    it("uses midpoint when P2 sliders are provided (couple mode)", () => {
      const p2Sliders = { groceries: 20, cooking: 20, cleaning: 20, admin: 20, childrenAppointments: 20, schoolSupport: 20, maintenance: 20, planning: 20 };
      ...
      expect(Math.abs(resultCouple.modelResult.p1Contribution - 3000 * (3200 / 5300)))
        .toBeLessThan(Math.abs(resultSolo.modelResult.p1Contribution - 3000 * (3200 / 5300)));
    });
    ...
    it("contributions sum to total charges", () => {
      const result = computeM5(base);
      expect(result.modelResult.p1Contribution + result.modelResult.p2Contribution)
        .toBeCloseTo(3000, 2);
    });
  });
  ```
...
- [ ] **Step 3: Implement m5-total-contribution.ts**
  ...
  export function computeM5(input: SimulationInput): M5Result {
    const mergedSliders = mergeDomesticSliders(
      input.domesticSliders.p1,
      input.domesticSliders.p2
    );

    const domestic = computeDomesticValue(
      mergedSliders,
      input.hasChildren,
      input.hourlyRate
    );
    ...
    const isSameAsM2 =
      Math.abs(ratioAfter - ratioIncomeBefore) < SAME_AS_M2_THRESHOLD;
...
- [ ] **Step 4: Write failing tests for calculate**
  ...
  describe("calculate — orchestrator", () => {
    ...
    it("m1 has the highest equity gap (lowest score) for unequal incomes", () => {
      const results = calculate(base);
      expect(results.m1_5050.modelResult.equityScore)
        .toBeLessThan(results.m2_income_ratio.equityScore);
    });
  ...
  export function calculate(input: SimulationInput): CalculationResults {
    ...
    return {
      ...
      projections: {
        ...
        m4_adjusted_time: savingsGapProjection(m4.optionB.p1DisposableIncome - m4.optionB.p2DisposableIncome),
        m5_total_contribution: savingsGapProjection(m5.modelResult.p1DisposableIncome - m5.modelResult.p2DisposableIncome),
      },
      validationErrors,
    };
  }
...
| Date | Status | Notes |
|------|--------|-------|
| 2026-03-17 | 🔲 Not started | Depends on Plan 01 complete |
